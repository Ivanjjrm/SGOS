...
try
  begin
    if (Eid.Text <> '') then
    if (Application.MessageBox('Deseja realmente excluir esse serviço?','Atenção Usuário',MB_YESNO+MB_ICONQUESTION)=6) then
      begin
        IBQuerycad.Close;
       ...
       on e: Exception do
      begin
        Application.MessageBox('Não é possivel Excluir! Existem referências deste serviço no sistema!','Atenção Usuário',MB_ICONERROR);
      end;
  end;
end;
