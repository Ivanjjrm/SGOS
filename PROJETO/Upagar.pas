...
begin
if ((DbGrid1.Columns.Items[12].Field).AsString = 'ABERTA') then
begin
...
begin
if ((DbGrid1.Columns.Items[12].Field).AsString <> 'ABERTA') and (IBQuerygrid.RecordCount > 0 ) then
begin
Application.MessageBox('Não é possivel excluir! A conta já está paga.','Erro',MB_ICONERROR);
end;
if ((DbGrid1.Columns.Items[12].Field).AsString = 'ABERTA') and (Application.MessageBox('Deseja realmente excuir?','Atenção Usuário',MB_YESNO+MB_ICONQUESTION)=6) and (IBQuerygrid.RecordCount > 0) then
begin
IBQueryExCont.Close;
...
begin
if (RadioGroup1.ItemIndex = 0) and (IBQuerygrid.RecordCount > 0) then
Begin
ppLabelDesc.Text:='Lista Contas Pagas de '+DateTostr(DateTimePicker1.date)+' até '+DateTostr(DateTimePicker2.date);
ppReportRelpag.Print;
End
else if (RadioGroup1.ItemIndex = 1) and (IBQuerygrid.RecordCount > 0) then
Begin
ppLabelDesc.Text:='Lista Contas A Pagar de '+DateTostr(DateTimePicker1.date)+' até '+DateTostr(DateTimePicker2.date);
ppReportRelpag2.Print;
End;
end;
...
begin
  if ((DbGrid1.Columns.Items[12].Field).AsString <> 'ABERTA') and (IBQuerygrid.RecordCount > 0 ) then
    begin
    Application.MessageBox('Não é Possivel Alterar Conta já Paga!','Erro',MB_ICONERROR);
    end
  else if ((DbGrid1.Columns.Items[12].Field).AsString = 'ABERTA') and (IBQuerygrid.RecordCount > 0) then
    begin
      ...
      FrmNovoPagar.DateTimePicker1.Date:=StrToDate((DbGrid1.Columns.Items[8].Field).AsString);
      FrmNovoPagar.DateTimePicker2.Date:=StrToDate((DbGrid1.Columns.Items[9].Field).AsString);
      FrmNovoPagar.Mref.Text:=(DbGrid1.Columns.Items[7].Field).AsString;
      FrmNovoPagar.ShowModal;
