object FrmItemOs: TFrmItemOs
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsSingle
  Caption = 'Ordem de Servi'#231'o'
  ClientHeight = 441
  ClientWidth = 671
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 8
    Width = 37
    Height = 13
    Caption = 'Cliente:'
  end
  object Label2: TLabel
    Left = 8
    Top = 47
    Width = 66
    Height = 13
    Caption = 'Equipamento:'
  end
  object Label3: TLabel
    Left = 8
    Top = 86
    Width = 94
    Height = 13
    Caption = 'Defeito Reclamado:'
  end
  object Label5: TLabel
    Left = 7
    Top = 143
    Width = 39
    Height = 13
    Caption = 'Servi'#231'o:'
  end
  object Label6: TLabel
    Left = 8
    Top = 184
    Width = 103
    Height = 13
    Caption = 'Descri'#231#227'o do Servi'#231'o:'
  end
  object Label9: TLabel
    Left = 225
    Top = 417
    Width = 37
    Height = 16
    Caption = 'Total:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label11: TLabel
    Left = 343
    Top = 143
    Width = 28
    Height = 13
    Caption = 'Valor:'
  end
  object Bevel1: TBevel
    Left = -1
    Top = 401
    Width = 546
    Height = 38
  end
  object Label12: TLabel
    Left = 400
    Top = 49
    Width = 66
    Height = 13
    Caption = 'Identifica'#231#227'o:'
  end
  object Bevel2: TBevel
    Left = 4
    Top = 7
    Width = 537
    Height = 133
  end
  object Label4: TLabel
    Left = 279
    Top = 49
    Width = 37
    Height = 13
    Caption = 'C'#243'digo:'
  end
  object Ecliente: TEdit
    Left = 8
    Top = 22
    Width = 529
    Height = 21
    Enabled = False
    TabOrder = 0
  end
  object MDefeito: TMemo
    Left = 8
    Top = 100
    Width = 529
    Height = 33
    MaxLength = 150
    TabOrder = 1
  end
  object DBGrid1: TDBGrid
    Left = 8
    Top = 243
    Width = 533
    Height = 157
    DataSource = DataSourceGrid
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDrawColumnCell = DBGrid1DrawColumnCell
    OnDblClick = DBGrid1DblClick
    Columns = <
      item
        Expanded = False
        FieldName = 'ID_ITEM'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'NOME_EQUIP'
        Title.Caption = 'Equipamento'
        Width = 160
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DEFEITO'
        Title.Caption = 'Defeito Reclamado'
        Width = 300
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'IDENTIFICACAO'
        Title.Caption = 'Identifica'#231#227'o'
        Width = 160
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESC_SERVICO'
        Title.Caption = 'Descri'#231#227'o do Servi'#231'o'
        Width = 350
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR'
        Title.Caption = 'Valor'
        Width = 80
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ID_SERV_FK'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'ID_EQUIP_FK'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'ID_OS_FK'
        Visible = False
      end>
  end
  object DBLookupComboBoxServico: TDBLookupComboBox
    Left = 8
    Top = 162
    Width = 321
    Height = 21
    KeyField = 'ID_SERV'
    ListField = 'DESC_SERV'
    ListSource = DataSourceServ
    TabOrder = 3
    OnClick = DBLookupComboBoxServicoClick
    OnExit = DBLookupComboBoxServicoExit
    OnKeyPress = DBLookupComboBoxServicoKeyPress
  end
  object MDescServ: TMemo
    Left = 8
    Top = 197
    Width = 434
    Height = 40
    CharCase = ecUpperCase
    MaxLength = 200
    TabOrder = 4
  end
  object Etotal: TEdit
    Left = 268
    Top = 409
    Width = 94
    Height = 24
    Alignment = taRightJustify
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 8
    Text = '0,00'
  end
  object Panel1: TPanel
    Left = 545
    Top = -6
    Width = 142
    Height = 459
    TabOrder = 7
    object Label10: TLabel
      Left = 8
      Top = 10
      Width = 44
      Height = 13
      Caption = 'Data O.S'
    end
    object BitBtn4: TBitBtn
      Left = 1
      Top = 318
      Width = 128
      Height = 42
      Caption = 'Fechar O.S'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 0
      OnClick = BitBtn4Click
    end
    object BitBtn5: TBitBtn
      Left = 1
      Top = 400
      Width = 128
      Height = 42
      Caption = 'Sair'
      Glyph.Data = {
        
        9999999999999999989898969696959595949494939393929292}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = BitBtn5Click
    end
    object BitBtn6: TBitBtn
      Left = 1
      Top = 359
      Width = 128
      Height = 42
      Caption = 'Cancelar O.S'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      TabOrder = 2
      OnClick = BitBtn6Click
    end
    object Eidos: TEdit
      Left = 24
      Top = 65
      Width = 57
      Height = 21
      TabOrder = 3
      Visible = False
    end
    object BitBtn7: TBitBtn
      Left = 2
      Top = 278
      Width = 128
      Height = 41
      Caption = 'Lan'#231'ar Servi'#231'o'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      TabOrder = 4
      OnClick = BitBtn7Click
      OnKeyPress = BitBtn7KeyPress
    end
  end
  object BitBtn1: TBitBtn
    Left = 448
    Top = 179
    Width = 89
    Height = 29
    Caption = 'Cancelar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    TabOrder = 9
    OnClick = BitBtn1Click
  end
  object BitBtn2: TBitBtn
    Left = 448
    Top = 148
    Width = 89
    Height = 29
    Caption = 'Lan'#231'ar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    TabOrder = 6
    OnClick = BitBtn2Click
    OnKeyPress = BitBtn2KeyPress
  end
  object BitBtn3: TBitBtn
    Left = 448
    Top = 210
    Width = 89
    Height = 29
    Caption = 'Excluir'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    TabOrder = 10
    OnClick = BitBtn3Click
  end
  object Edata: TEdit
    Left = 549
    Top = 22
    Width = 112
    Height = 21
    Enabled = False
    TabOrder = 11
  end
  object Evalor: TEdit
    Left = 351
    Top = 162
    Width = 91
    Height = 21
    TabOrder = 5
    OnExit = EvalorExit
    OnKeyPress = EvalorKeyPress
  end
  object Eidenfic: TEdit
    Left = 400
    Top = 63
    Width = 137
    Height = 21
    TabOrder = 12
  end
  object Eiditem: TEdit
    Left = 279
    Top = 63
    Width = 115
    Height = 21
    Enabled = False
    TabOrder = 13
  end
  object DBLookupComboBoxEquipamento: TDBLookupComboBox
    Left = 8
    Top = 63
    Width = 265
    Height = 21
    KeyField = 'ID_EQUIP'
    ListField = 'NOME_EQUIP'
    ListSource = DataSourceEquip
    TabOrder = 14
  end
  object IBQueryEquip: TIBQuery
    Database = Form1.IBDatabase
    Transaction = Form1.IBTransaction
    Active = True
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'select * from EQUIPAMENTO'
      'ORDER BY  NOME_EQUIP')
    Left = 40
    Top = 104
  end
  object IBQueryServ: TIBQuery
    Database = Form1.IBDatabase
    Transaction = Form1.IBTransaction
    Active = True
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT * FROM SERVICO'
      'ORDER BY DESC_SERV')
    Left = 112
    Top = 152
  end
  object DataSourceEquip: TDataSource
    DataSet = IBQueryEquip
    Left = 160
    Top = 104
  end
  object DataSourceServ: TDataSource
    DataSet = IBQueryServ
    Left = 176
    Top = 232
  end
  object IBQueryGrid: TIBQuery
    Database = Form1.IBDatabase
    Transaction = Form1.IBTransaction
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT I.ID_ITEM,I.DEFEITO,I.IDENTIFICACAO,I.DESC_SERVICO,'
      '       I.VALOR,I.ID_SERV_FK,I.ID_EQUIP_FK,I.ID_OS_FK,'
      '       E.NOME_EQUIP FROM ITEM_OS'
      
        '       AS I INNER JOIN EQUIPAMENTO AS E ON ID_EQUIP_FK = ID_EQUI' +
        'P')
    Left = 184
    Top = 312
    object IBQueryGridID_ITEM: TIntegerField
      FieldName = 'ID_ITEM'
      Origin = '"ITEM_OS"."ID_ITEM"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object IBQueryGridDEFEITO: TIBStringField
      FieldName = 'DEFEITO'
      Origin = '"ITEM_OS"."DEFEITO"'
      Size = 150
    end
    object IBQueryGridIDENTIFICACAO: TIBStringField
      FieldName = 'IDENTIFICACAO'
      Origin = '"ITEM_OS"."IDENTIFICACAO"'
      Size = 60
    end
    object IBQueryGridDESC_SERVICO: TIBStringField
      FieldName = 'DESC_SERVICO'
      Origin = '"ITEM_OS"."DESC_SERVICO"'
      Size = 200
    end
    object IBQueryGridVALOR: TIBBCDField
      FieldName = 'VALOR'
      Origin = '"ITEM_OS"."VALOR"'
      Precision = 9
      Size = 2
    end
    object IBQueryGridID_SERV_FK: TIntegerField
      FieldName = 'ID_SERV_FK'
      Origin = '"ITEM_OS"."ID_SERV_FK"'
    end
    object IBQueryGridID_EQUIP_FK: TIntegerField
      FieldName = 'ID_EQUIP_FK'
      Origin = '"ITEM_OS"."ID_EQUIP_FK"'
    end
    object IBQueryGridID_OS_FK: TIntegerField
      FieldName = 'ID_OS_FK'
      Origin = '"ITEM_OS"."ID_OS_FK"'
    end
    object IBQueryGridNOME_EQUIP: TIBStringField
      FieldName = 'NOME_EQUIP'
      Origin = '"EQUIPAMENTO"."NOME_EQUIP"'
      Size = 50
    end
  end
  object DataSourceGrid: TDataSource
    DataSet = IBQueryGrid
    Left = 352
    Top = 200
  end
  object IBQueryLanc: TIBQuery
    Database = Form1.IBDatabase
    Transaction = Form1.IBTransaction
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    Left = 264
    Top = 200
  end
end
